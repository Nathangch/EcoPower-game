...
class GameState:
    def __init__(self):
        ...
        self.creative_item = "Cabo Alumínio" # For placing items in Creative Mode
            ...
            for source in ordered_sources:
                if source.energy_atual < source.capacidade_atual: # Use capacidade_atual
                    ...
                    if path:
                        ...
                        # find target city at end of path
                        current = source
                        for edge in path:
    ...
    def _check_boss_condition(self):
        ...
        # If no energy is reaching any cities but demand exists, and multiple failures happened
        total_delivered = sum(c.energy_atual for c in self.nodes if c.type == "cidade")
        if total_delivered == 0 and len(self.edges) < 2: # This condition seems a bit arbitrary for "multiple failures"
...
class App(tk.Tk):
    ...
    def show_game(self):
        ...
        # If creative mode, nodes/edges are already cleared by start_creative_mode
    ...
    def setup_ui(self):
        ...
        # Main Layout
        content = tk.Frame(self.current_frame, bg="#2E3440")
        content.pack(side=tk.TOP, fill=tk.BOTH, expand=True)
        
                # Sidebar for Creative Mode (Categorized)
        if self.game.mode == "CRIATIVO":
            self.setup_creative_sidebar(content)
        
        # Top Panel
        top_frame = tk.Frame(content, bg="#3B4252", height=60)
        ...
        self.btn_power.pack(side=tk.LEFT, padx=10, pady=10)
        ...
        if self.game.mode == "SUSTENTAVEL":
            ...
            lbl_sus = tk.Label(top_frame, text=f"Sustentabilidade: {sus_idx}%", bg="#3B4252", fg="#A3BE8C" if sus_idx > 70 else "#BF616A", font=("Arial", 12, "bold"))
            lbl_sus.pack(side=tk.RIGHT, padx=20)
        
        if self.game.mode == "EDUCACIONAL":
            lbl_fase = tk.Label(top_frame, text=f"FASE {self.game.current_level}/25", bg="#3B4252", fg="#A3BE8C", font=("Arial", 14, "bold"))
            lbl_fase.pack(side=tk.RIGHT, padx=20)
        ...
        # Canvas
        self.canvas = tk.Canvas(content, bg="#2E3440", highlightthickness=0)
        ...
        if self.game.mode in ["EDUCACIONAL", "SUSTENTAVEL"]:
            txt = "❓ Como Jogar? (Educativo)" if self.game.mode == "EDUCACIONAL" else "🍃 Guia de Sustentabilidade"
    ...
    def show_category(self, cat_name):
        ...
        for item in self.categories[cat_name]:
            btn = tk.Button(self.item_frame, text=item, bg="#434C5E", fg="#ECEFF4", font=("Arial", 11, "bold"),
    ...
    def reset_level(self):
        ...
        # update the phase label and difficulty label if they exist
        for widget in self.current_frame.winfo_children():
            if isinstance(widget, tk.Frame): # top frame
                for w in widget.winfo_children():
                    if isinstance(w, tk.Label) and "FASE" in w.cget("text"):
                        w.config(text=f"FASE {self.game.current_level}/25")
                    elif isinstance(w, tk.Label) and "Sustentabilidade" in w.cget("text"):
                        sus_idx = self.game.get_sustainability_index()
                        w.config(text=f"Sustentabilidade: {sus_idx}%", fg="#A3BE8C" if sus_idx > 70 else "#BF616A")
    ...
    def on_left_click(self, event):
        ...
        # Creative Mode - Tools & Place Item
        if self.game.mode == "CRIATIVO":
            ...
            # Placement
            if "Cabo" not in item and item != "Supercondutor":
                if item == "Solar":
                    ...
                    self.game.nodes.append(Substation(x, y, f"Sub {chr(65+len(self.game.nodes))}"))
                elif item == "Transformador":
                    self.game.nodes.append(Node(x, y, f"Trf {len(self.game.nodes)+1}", "subestacao")) # Reusing subestacao visual for now
        ...
        if clicked_node:
            if self.selected_node == None:
                ...
                if not exists:
                    ...
                    cap = 5
                    c_type = "ALUMINIO" # Default for educational
                    if self.game.mode == "CRIATIVO":
                        item = self.game.creative_item
                        if item == "Cabo Média":
                            cap, c_type = 15, "MEDIA"
    ...
    def on_right_click(self, event):
        ...
        if edge:
            ...
            self.game.update_flow()
            self.lbl_info.config(text="Cabo removido.")
            self.draw_grid()
        else: # If no edge, try to remove a node in creative mode
            if self.game.mode == "CRIATIVO":
    ...
    def draw_legend(self):
        ...
        for i, (text, color, shape) in enumerate(items):
            ...
            if shape == "rect":
                ...
                self.canvas.create_oval(lx1+10, py-7, lx1+25, py+7, outline=color, width=2)
            elif shape == "poly":
                self.canvas.create_polygon(lx1+10, py+5, lx1+17, py-7, lx1+25, py+5, fill="#81A1C1", outline="#ECEFF4")
            elif shape == "line":
